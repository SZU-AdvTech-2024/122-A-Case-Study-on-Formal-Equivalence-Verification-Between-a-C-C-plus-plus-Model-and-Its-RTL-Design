...
//Widden Begin
//rv_vwredsum_vs
ui128 rv_vwredsum_vs(ui128 vs1, ui128 vs2, ui128 vm, ui128 oldvd, ui8 vl, ui3 vsew, bool vma, bool vta, bool Mask) {
  ui128 vd = 0;
  if (vsew == 0x0) {
     ui8  vs2_arr;
     ui16 vs1_arr, vs2_arr_ext, vd_arr, oldvd_arr;
     vd_arr  = vs1.slc<16>(0);
     for (uint i = 0; i < 8; i++) {
        vs2_arr   = vs2.slc<8>(i*8);
        if(vs2.slc<1>((i+1)*8-1)){
          vs2_arr_ext = 0xFF00|vs2_arr;
        }else{
          vs2_arr_ext = 0x0000|vs2_arr;
        }
        if (i < vl) {
           if ((Mask == 1) && (vm[i] == 0)) {
           } else {
              vd_arr  = vd_arr + vs2_arr_ext;
           }
        }
     }
     if (vl != 0) {
  ...
  if (vsew == 0x1) {
     ui16 vs2_arr;
     ui32 vs1_arr, vs2_arr_ext, vd_arr, oldvd_arr;
     vd_arr  = vs1.slc<32>(0);
     for (uint i = 0; i < 4; i++) {
        vs2_arr   = vs2.slc<16>(i*16);
        if(vs2.slc<1>((i+1)*16-1)){
          vs2_arr_ext = 0xFFFF0000|vs2_arr;
        }else{
          vs2_arr_ext = 0x00000000|vs2_arr;
        }
        if (i < vl) {
           if ((Mask == 1) && (vm[i] == 0)) {
           } else {
  ...
  if (vsew == 0x2) {
     ui32 vs2_arr;
     ui64 vs1_arr, vs2_arr_ext, vd_arr, oldvd_arr;
     vd_arr  = vs1.slc<64>(0);
     for (uint i = 0; i < 2; i++) {
        vs2_arr   = vs2.slc<32>(i*32);
        if(vs2.slc<1>((i+1)*32-1)){
          vs2_arr_ext = 0xFFFFFFFF00000000|vs2_arr;
        }else{
          vs2_arr_ext = 0x0000000000000000|vs2_arr;
        }
        if (i < vl) {
           if ((Mask == 1) && (vm[i] == 0)) {
           } else {
...
//rv_vwredsumu_vs
ui128 rv_vwredsumu_vs(ui128 vs1, ui128 vs2, ui128 vm, ui128 oldvd, ui8 vl, ui3 vsew, bool vma, bool vta, bool Mask) {
  ui128 vd = 0;
  if (vsew == 0x0) {
     ui8  vs2_arr;
     ui16 vs1_arr, vs2_arr_ext, vd_arr, oldvd_arr;
...
void hector_wrapper()
{
    ...
    vd = 0;
    if((funct6 == 0x31) && (funct3 == 0x0) && (Widden == 0x1) && (vstart == 0x0) && (eewType_1 == 0x3)) { vd = rv_vwredsum_vs(vs1, vs2, vm, oldvd, vl, vsew, vma, vta, Mask); }
    if((funct6 == 0x30) && (funct3 == 0x0) && (Widden == 0x1) && (vstart == 0x0) && (eewType_1 == 0x3)) { vd = rv_vwredsumu_vs(vs1, vs2, vm, oldvd, vl, vsew, vma, vta, Mask); }
